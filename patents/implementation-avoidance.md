...
### 2.2 消費者コード読取による紐付け（CPM方式）

**関連特許:** 東芝テック関連特許

| 非推奨事項 | 詳細 |
|-----------|------|
| 利用者QR表示→店舗読取 | 利用者がスマホでQRコード表示し、店舗POSが読み取る方式 |

**代替実装案（推奨）:**
- **MPM方式**: 店舗がQRコード表示→利用者がスマホで読取
- **メールアドレス入力方式**: 店舗端末でメールアドレスを入力し送信
- **ワンタイムトークン**: URLに埋め込んだ一時トークンで認証
...
- [ ] 利用者QR表示→店舗読取の方式を採用していないか
- [ ] 店舗QR表示→利用者読取の方式を採用しているか（推奨）
...
- [patent-landscape-summary.md](patent-landscape-summary.md) - 調査結果サマリー
- [report-toshiba-tec.md](report-toshiba-tec.md) - 東芝テック特許詳細
- [report-freee.md](report-freee.md) - freee特許詳細
